...
;(function (root, factory) {
  ...
  jet.returnDate = function (obj, format, dateval) {
    ...
    var undate = (dateval == undefined || dateval == "" || dateval == []), darr = undate ? [] : jet.reMacth(dateval),
        sparr = [];
    if (!undate) {
      darr[1] -= 1;
      darr[1] = darr[1] == -1 ? 12 : darr[1];
    }
    var myDate = darr.length > 0 ? new Date(darr[0], darr[1], (darr[2] || 0), (darr[3] || 0), (darr[4] || 0), (darr[5] || 0)) : new Date(),
    ...
    var mday = jet.getDaysNum(narr[0], narr[1] + 1),
        ...
        parnaVal = narr[2] + parseInt(sparr[2] || 0), gday, reday;
    //判断今天是否为31号
    gday = isDay31 ? (parnaVal - 1) : parnaVal;
    //重新设置日期，必须要用new Date来设置，否则就会有问题
    var setdate = new Date(narr[0] + parseInt(sparr[0] || 0), narr[1] + parseInt(sparr[1] || 0), gday, narr[3] + parseInt(sparr[3] || 0), narr[4] + parseInt(sparr[4] || 0), narr[5] + parseInt(sparr[4] || 0));
  ...
  jedfn.eventsDate = function (opts, boxCell) {
    ...
    boxCell.find(".jedatebot .jedateok").on("click", function (ev) {
      ...
      var okymd = ishhmat ? [date.getFullYear(), date.getMonth() + 1, date.getDate()] : jet.reMacth(datevalue),
          ...
          okVal = jet.parse([parseInt(okymd[0]), parseInt(okymd[1]), parseInt(okymd[2])], [okhms[0] || 0, okhms[1] || 0, okhms[2] || 0], okformat),
          okdate = (okymd[0] || date.getFullYear()) + "-" + jet.digit(okymd[1] || date.getMonth() + 1) + "-" + jet.digit(okymd[2] || date.getDate()) + " " + jet.digit(okhms[0] || 0) + ":" + jet.digit(okhms[1] || 0) + ":" + jet.digit(okhms[2] || 0);
