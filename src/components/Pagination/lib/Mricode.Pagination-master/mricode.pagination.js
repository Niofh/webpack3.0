 ...
 */
//http://www.2cto.com/kf/201507/417874.html
(function (factory) {
  ...
  var Page = function (element, options) {
    ...
    this.$kuozhang = $('<div class="m-pagination-kuozhang">条</div>').hide();
  ...
  Page.prototype = {
    ...
    renderPagination: function () {
      ...
      if (this.options.showPageSizes && lastPageNum !== 0) this.$kuozhang.show(); else this.$kuozhang.hide();
  ...
  pagination.remote = {
    getAjax: function (pagination, url, data, success, beforeSend, complate, traditional, async) {
  ...
  pagination.core = {
    ...
    renderPages: function (pageIndex, pageSize, total, pageBtnCount, options) {
      options = options || {};
      var pageNumber = pageIndex + 1;  //当前页数
      var lastPageNumber = this.calLastPageNum(total, pageSize); //分也总数
      ...
      var prevPage = this.renderPerPage(options.prevBtnText, (pageIndex - 1 < 1 ? 0 : pageIndex - 1));
      var nextPage = this.renderPerPage(options.nextBtnText, pageIndex + 1 >= lastPageNumber ? lastPageNumber - 1 : pageIndex + 1);
